...
static public class Easing
{
    ...
    public delegate float EaseFunc(float s, float e, float v);
    ...
    public static EaseFunc GetEasingFunction(Ease easingFunction)
    {
    ...
    public static EaseFunc GetEasingFunctionDerivative(Ease easingFunction)
    {
        if (easingFunction == Ease.EaseInQuad)
        {
            return EaseInQuadD;
    ...
    static Easing()
    {
        var easingCount = typeof(Ease).GetEnumNames().Length;
        var easings = new EaseFunc[easingCount];
        for (int i = 0; i < easingCount; i++)
            easings[i] = GetEasingFunction((Ease)i);
        EasingFuncs = Array.AsReadOnly(easings);
    }
