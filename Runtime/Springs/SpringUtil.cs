    ...
    public static class SpringUtil
    {
        ...
        ******************************************************************************/
        ...
        private static void UpdateDampedSpringMotion(
            ...
            float goalPos, // position to approach
            DampedSpringMotionParams parameters)         // motion parameters to use
        {
            float oldPos = pPos - goalPos; // update in equilibrium relative space
            float oldVel = pVel;

            pPos = oldPos * parameters.posPosCoef + oldVel * parameters.posVelCoef + goalPos;
        ...
        /// <param name="goalPosition">Goal (or rest) position</param>
        ...
        public static void CalcDampedSimpleHarmonicMotion(ref float position, ref float velocity,
            float goalPosition, float deltaTime, float angularFrequency, float dampingRatio)
        {
            var motionParams = CalcDampedSpringMotionParams(deltaTime, angularFrequency, dampingRatio);
            UpdateDampedSpringMotion(ref position, ref velocity, goalPosition, motionParams);
        }
        ...
        public static void CalcDampedSimpleHarmonicMotion(ref Vector2 position, ref Vector2 velocity,
            Vector2 goalPosition, float deltaTime, float angularFrequency, float dampingRatio)
        {
            var motionParams = CalcDampedSpringMotionParams(deltaTime, angularFrequency, dampingRatio);
            UpdateDampedSpringMotion(ref position.x, ref velocity.x, goalPosition.x, motionParams);
            UpdateDampedSpringMotion(ref position.y, ref velocity.y, goalPosition.y, motionParams);
        }
        ...
        public static void CalcDampedSimpleHarmonicMotion(ref Vector3 position, ref Vector3 velocity,
            Vector3 goalPosition, float deltaTime, float angularFrequency, float dampingRatio)
        {
            var motionParams = CalcDampedSpringMotionParams(deltaTime, angularFrequency, dampingRatio);
            UpdateDampedSpringMotion(ref position.x, ref velocity.x, goalPosition.x, motionParams);
            UpdateDampedSpringMotion(ref position.y, ref velocity.y, goalPosition.y, motionParams);
            UpdateDampedSpringMotion(ref position.z, ref velocity.z, goalPosition.z, motionParams);
        }
    }
}
